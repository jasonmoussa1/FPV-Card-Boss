...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function runGoProRobot(coords: any, rawPath: string, stabilizedPath: string, goProPath: string, goProOutputPath?: string, meta?: { cardId: string; pilotName: string; artistName: string; horizonLock?: boolean; dual?: boolean }): Promise<{ success: boolean; message: string; robotStartTime?: number }> {
  if (window.electron) {
